...
function reverie_css() {
  if ( !is_admin() ) {
     ...
     wp_register_style( 'righteous_font', 'http://fonts.googleapis.com/css?family=Righteous', false );
     wp_enqueue_style( 'righteous_font' );
...
global $is_IE;
  ...
  // Enqueue to header
     ...
     wp_register_script( 'placeholder', get_template_directory_uri() . '/js/jquery.textPlaceholder.js', array( 'jquery' ) );
     wp_enqueue_script( 'placeholder' );

     wp_register_script( 'slidedeck', get_template_directory_uri() . '/js/jquery.slidedeck.js', array( 'jquery' ) );
     wp_enqueue_script( 'slidedeck' );
 // Enqueue to footer
...
function day_of_year($date_from_press) {
  $written = date_parse($date_from_press);
  $months = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
  return (string)$months[$written['month']-1] . ' ' . (string)$written['day'] . ', ' . (string)$written['year'];
}

add_action( 'press-page', 'day_of_year');
...
require_once("inc/fellows.php");
require_once("inc/home-images.php");
require_once("inc/partners.php");
require_once("inc/press.php");
require_once("inc/team.php");
require_once("inc/blog.php");
require_once("inc/faqs.php");


function excerpt_ellipse($text) {
   return str_replace('[...]', ' <a class="read-more" href="'.get_permalink().'">[...]</a>', $text); }
